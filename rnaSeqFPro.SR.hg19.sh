...
touch script.deseq2.R

echo "#!/usr/bin/Rscript" > script.deseq2.R
echo "library(DESeq2)" >> script.deseq2.R
echo "data<-read.delim(\"mastertable.genename\", header=T, row.names = 1, check.names=F)" >>script.deseq2.R
echo "meta<-read.delim(\"meta.data\", header=T)" >>script.deseq2.R
###
echo "conds<-factor(meta\$Condition)
sampleTable<-data.frame(sampleName=colnames(data), condition=conds)
countsTable = data
...
dds <- DESeqDataSetFromMatrix(countData = countsTable, colData = meta, design = ~ Condition)
...
write.csv(as.data.frame(resSig), file=\"condition_treated_signif.csv\")

# Make a basic volcano plot
with(res, plot(log2FoldChange, -log10(pvalue), pch=20, main="Volcano plot", xlim=c(min(res$log2FoldChange[is.finite(res$log2FoldChange)]),max(res$log2FoldChange[is.finite(res$log2FoldChange)])), ylim=c(0,max(-log10(res$pvalue)[is.finite(-log10(res$pvalue))]))))
...
# Label points with the textxy function from the calibrate plot
library(calibrate)
with(subset(res, padj<.05 & abs(log2FoldChange)>1), textxy(log2FoldChange, -log10(pvalue), labs=id, cex=.8))

" >> script.deseq2.R

chmod 775 script.deseq2.R
./script.deseq2.R
rm script.deseq2.R
